...
function animationLoop(){
    ...
    bee(beeobj); //--bee
    if(beeobj.angle>6){beeobj.change.angle=-1} else if (beeobj.angle<-6){beeobj.change.angle=1} //wiggles
...
function splat(o){
    const pi=Math.PI;
    ctx.beginPath();
    ctx.arc(o.x, h-o.y, o.size, 0, 2*pi); //main splat
    ctx.fillStyle = "hsla("+o.color+", 100%, 50%, 1)";
    ctx.fill();
    for (let i=0;i<3;i++){
        ctx.beginPath();
        ctx.arc(o.x+o.change.size, o.y+o.change.size, 0.7*o.change.size, 0, 2*pi); //small splat
        ctx.arc(o.x+2*o.change.size, o.y+2*o.change.size, 0.3*o.change.size, 0, 2*pi); //tiny splat
        ctx.fill();
    } 
}
