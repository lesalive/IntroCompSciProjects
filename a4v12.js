...
let intervalFlr;
let intervalFgrowth;
const beecolors = ["yellow", "gold", "goldenrod", "darkgoldenrod", "rosybrown", "sienna", "burlywood", "peachpuff", "lightsalmon"]
let beeobj = {
    x:w/2,
    y:h/2,
    w:30,
...
let rainParticles = []
let hexcode = ["f", "e", "d", "c", "b", "a", "9", "8", "7", "6", "5", "4", "3", "2", "1", "0"] //change cloud alpha next
//-----
...
function animationLoop(){
    ...
    if (cloudobj.on==true){            //--rain if on
        rainCloud(cloudobj); //pushes particle
        for(let i =0; i<rainParticles.length; i++){//draws rain
           rect(rainParticles[i]);
           updateProperties(rainParticles[i]);
           bounce(rainParticles[i]);   
        }
        cloudobj.color = "#ffffff" + hexcode[cloudobj.a2] + hexcode[cloudobj.a1];
        if (cloudobj.color=="#ffffff00"){   
        } else
        if (cloudobj.a1<15){
            cloudobj.a1++;
        } else {
        ...
        //console.log(cloudobj.color);
    }
    
    rect(cloudobj);                        //--cloud
    updateProperties(cloudobj);
    toriod(cloudobj);
...
function click(event){//when mouse is clicked
    ...
    if( //rain cloud
        event.offsetX > cloudobj.x-cloudobj.w/2 &&
        event.offsetX < cloudobj.x+cloudobj.w/2 &&
        event.offsetY > cloudobj.y-cloudobj.h/2 &&
        event.offsetY < cloudobj.y+cloudobj.h/2
    ){
        //console.log("click");
        cloudobj.on=true;
...
function cursorBee(beeobj, event){
...
function bounce(o){
    if(o.x<0){
        o.change.x *= -1;
        console.log("r"); //why isn't it logging
        o.ground++; //oh i realized i should've put it in the y one
    } else
    if (o.y>h || o.y<0){
        o.change.y *= -1;
    }
