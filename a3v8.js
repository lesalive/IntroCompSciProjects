...
setTimeout(function(){  // snow
    for (let i=0; i<groupSnow.length; i++){
        groupSnow[i].change.alpha = -1/(60*20); //alpha reduces to fade out after 20s
...
function animationLoop(){ // main animation
    ...
    for (let i=0; i<groupClouds.length; i++){ //----clouds----
        ...
        groupClouds[i].alpha = 1/bg.index*1000;    //reduce alpha
    }
...
function sky(bg){ //changes colour of sky   
    ...
    if (bg.alpha<1){bg.alpha += 1/(60*2);}                //change bg alpha
}
function createData(n){ //creates objects for rect snow array
    var array=[];
    for (let i=0; i<n; i++){
        array.push({    
            x: randi(w),
            ...
            angle: 45,
            rd:0,
            change : {x:randn(2), y:rand(5)+1, w:20/(60*6), h:20/(60*6), color:0, alpha:1/(60*6), rd:0.05}
        })
    }
    return array
}
...
function createDataClouds(n){
    ...
    for (let i=0; i<n; i++){
        array.push({
            ...
            light: randi(10),
            alpha: 1,
            change : {x:randn(5), y:0, s:0, alpha:0}
        })
    }
    return array
}
