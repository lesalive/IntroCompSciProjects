...
function keydown(event){ //if _ keys are pressed
    ...
    if (state.pause){}else //if paused don't
    ...
    if(event.key == "x"){
        allSplat.splice(0, allSplat.length);
    }
}
function click(event){//when mouse is clicked
    if (state.pause){state.follow=false;} else //when paused, follow must be turned off
