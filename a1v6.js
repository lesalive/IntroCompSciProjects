...
function clearCanvas(){
    if (firecount>=48){
        cx.clearRect(0,0,800,450);
        firecount=0;
    } else {
         alert("Not enough fires: Add more fires to clear the board");
        }
}
...
function drawBg(){
    ...
    cx.beginPath();
    cx.rect(50+xoffset,getY(),getX(),y);
    ...
    for (let h=0;h<y;h++){
        for (let i=0; i<x; i++){
        cx.rect(75+i*50+xoffset,getY()+20+h*80,25,50);
    }}
    cx.fillStyle=grdWin;
    cx.fill();
    cx.stroke();
...
if (firecount==48){
    ...
    cx.strokeStyle="red";
    cx.strokeText("That's a lot of Fires!!!", 20, 20);
}
}