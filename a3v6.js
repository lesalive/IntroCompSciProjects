...
//animationLoop();
sky(bg);
for (let i=0; i<groupClouds.length; i++){
    clouds(groupClouds[i]);
}
//----- functions
...
function createData(n){ //creates objects for rect snow array
    ...
    for (let i=0; i<n; i++){
        array.push({    
            ...
            angle: 45,
            rd:0,
            change : {x:randn(2), y:rand(5)+1, w:-0.01, h:-0.01, color:0, alpha:-1/(60*20), rd:0.05}
        })
    }
    return array
}
...
function clouds(o){ //draws cloud shape
    ...
    ctx.fillStyle = "hsla(0, 0%, 100%,"+o.alpha+")"
    ctx.fill();
}
