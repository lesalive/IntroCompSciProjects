...
let rainParticles = []
let hexcode = ["f", "e", "d", "c", "b", "a", "9", "8", "7", "6", "5", "4", "3", "2", "1", "0"]
//-----
...
function click(event){//when mouse is clicked
    ...
    function clickRect(event, obj){
        if(
            event.offsetX > obj.x-obj.w/2 &&
            event.offsetX < obj.x+obj.w/2 &&
            event.offsetY > obj.y-obj.h/2 &&
            event.offsetY < obj.y+obj.h/2
        ){
            console.log("click")
            obj.clicked=true;
...
function rainCloud(obj){
    if (obj.clicked=true || rainParticles.length<200){
            rainParticles.push({
                x: obj.x+randn(obj.w),
                y: obj.y,
                w: 2,
